...
- **`ARCHESTRA_CHAT_<PROVIDER>_API_KEY`** - LLM provider API keys for the built-in Chat feature.
  - Supported `<PROVIDER>` values: `ANTHROPIC`, `OPENAI`, `OPENROUTER`, `GEMINI`, `CEREBRAS`, `COHERE`, `GROQ`, `XAI`, `MISTRAL`, `PERPLEXITY`, `VLLM`, `OLLAMA`, `ZHIPUAI`, `DEEPSEEK`, `GITHUB_COPILOT`, `MICROSOFT_COPILOT`, `BEDROCK`, `MINIMAX`, `AZURE_OPENAI`
  ...
  - Note: `ARCHESTRA_CHAT_MICROSOFT_COPILOT_API_KEY` holds an Entra refresh token of an account with an M365 Copilot license, not a static API key
