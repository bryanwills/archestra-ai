...
`tracing` (facade), `serde` (derive), `enumset` (the `Effect` set is an
`EnumSet<Effect>` bitset), `thiserror` (the two error types). Dev-only:
`tracing-subscriber`, `serde_json`, `proptest`. Concepts and semantics live in
...
- **serde capability line**: pure data (`Label`, `Grant`, `AuditEntry`,
  contracts, dimensions, violations) derives `Serialize + Deserialize`;
  `Decision`/`Permit`/`BlockReason`/`TrajectoryId` are `Serialize`-only. Never
  add `Deserialize` to `Permit` (or `Decision`/`TrajectoryId`): a permit has no
  public constructor by design, so deserializing one forges the linear
  capability, and a deserialized `TrajectoryId` could alias a live trajectory.
