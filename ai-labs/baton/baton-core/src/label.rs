    ...
    pub fn lift(&self, grant: &Grant) -> Self {
        ...
        let effects = match &grant.effects {
            Some(waived) => match &self.effects {
                Effects::Declared(present) => Effects::Declared(*present - *waived),
...
pub struct Grant {
    ...
    pub effects: Option<EnumSet<Effect>>,
...
mod tests {
    ...
    proptest! {
        ...
        fn lift_is_inflationary_in_the_adequacy_order(x in arb_label(), g in arb_grant()) {
            let up = x.lift(&g);
            prop_assert!(trust_rank(up.trust) >= trust_rank(x.trust), "trust demoted");
            prop_assert!(audience_le(&x.audience, &up.audience), "audience demoted");
            prop_assert!(effects_le(&x.effects, &up.effects), "effects demoted");
        }
