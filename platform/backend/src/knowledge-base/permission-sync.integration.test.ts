...
describe("permission-sync end-to-end (GitHub, auto-sync-permissions)", () => {
  ...
  async function userAclFor(params: {
    ...
    const groupTokens = await KbExternalUserGroupModel.findGroupTokensForUser({
  ...
  test("permitted user retrieves the chunk; others do not; admin bypasses; revoke fail-closes", async ({
    ...
    // ---- First pass: tag the repo's docs with the collaborator's ACL ----
    const first = await permissionSyncService.executePass(connector.id);
    expect(first.status).toBe("success");
    expect(await chunkAcl(doc.id)).toEqual([`user_email:${PERMITTED_EMAIL}`]);
    ...
    // The chunk's ACL fail-closes; the permitted user no longer sees it.
    expect(await chunkAcl(doc.id)).toEqual([]);
    expect(
      await queryChunks({ connectorId: connector.id, userAcl: permittedAcl }),
    ).toHaveLength(0);
  ...
  test("group-based access resolves via the group snapshot at query time", async ({
    ...
    // The document carries the namespaced group token.
    expect(await chunkAcl(doc.id)).toEqual([`group:github_${OWNER}/eng`]);
    ...
    const rows = await db
      ...
      .where(
        and(
          eq(schema.kbExternalUserGroupTable.connectorId, connector.id),
          eq(schema.kbExternalUserGroupTable.memberEmail, PERMITTED_EMAIL),
        ),
      );
